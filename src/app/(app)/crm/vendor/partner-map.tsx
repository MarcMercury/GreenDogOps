...
// Dots are colored by how badly a partner needs a visit — driven by how long
// since the last visit — rather than by status.
function heatFor(p: CrmOrganization) {
  return visitHeat({ lastVisitDate: p.last_visit_date });
}

function lastVisitLabel(p: CrmOrganization): string {
  if (!p.last_visit_date) return "No visit on record";
  return `Last visit ${new Date(p.last_visit_date).toLocaleDateString()}`;
}

/** Full single-line address — must match the server geocoder's cache key. */
function fullAddress(p: CrmOrganization): string {
...
export function PartnerMap({
  ...
  const hasCoords = useCallback(
    (p: CrmOrganization) => typeof p.latitude === "number" && typeof p.longitude === "number",
    [],
  );
        ...
        if (info) {
          ...
          info.setContent(
            `<div style="font:13px/1.4 system-ui,sans-serif;max-width:230px">
              <a href="/crm/org/${escapeHtml(p.id)}" style="font-weight:600;color:#047857;text-decoration:none;cursor:pointer;margin-bottom:2px;display:inline-block">${escapeHtml(p.name)}</a>
              ...
              <div style="color:#64748b;font-size:12px;margin-bottom:${addr ? "2px" : "6px"}">${escapeHtml(lastVisitLabel(p))}</div>
              ${addr ? `<div style="color:#64748b;font-size:12px;margin-bottom:6px">${escapeHtml(addr)}</div>` : ""}
  ...
  function runGeocode() {
    startGeocode(async () => {
      const r = await geocodePartnerOrgs();
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 text-xs text-slate-500">
            ...
            {canEdit && needsGeocode.length > 0 && (
              ...
              >
                {geocoding ? "Plotting…" : `📍 Plot ${needsGeocode.length} partner${needsGeocode.length === 1 ? "" : "s"}`}
              </button>
            )}
          </div>
        </div>
      ...
      {status === "ready" && (visible.length === 0 || noAddress > 0) && (
        <p className="text-xs text-slate-400">
          {visible.length === 0 && "No partners match the current filters. "}
          {noAddress > 0 && `${noAddress} partner${noAddress === 1 ? "" : "s"} have no address on file and can't be mapped.`}
        </p>
      )}
    </div>
  );
