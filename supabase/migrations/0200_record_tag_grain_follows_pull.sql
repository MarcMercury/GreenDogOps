-- ============================================================================
-- Green Dog Ops — 0200 record tag grain follows the pull
-- ----------------------------------------------------------------------------
-- merge_record_tag() upserted the catalog row but left record_type alone, so a
-- tag seeded with a guessed grain kept that guess even after being loaded at a
-- different one. That matters because the grain decides which record's modified
-- date the nightly activity window is judged against — and if the catalog and
-- the stored membership disagree, the incremental run reads the wrong table,
-- retires nothing, and the tag quietly stops updating.
--
-- The grain the data actually arrived at is the truth, so the pull now sets it.
-- ============================================================================
...
-- These four arrived as CONTACT-grain exports, so drop the seeded guess that
-- they were pet tags pulled at pet grain.
update greendogops.ezyvet_tag
   set tag_type = 'contact_tag', record_type = 'contact', tag_group = 'General'
 where tag_key in ('code_101', 'code_501');

insert into greendogops.ezyvet_tag (tag_key, tag_label, tag_type, tag_group, record_type)
values
  ('ap', 'ap', 'contact_tag', 'General', 'contact'),
  ('alert', 'Alert', 'contact_tag', 'General', 'contact')
